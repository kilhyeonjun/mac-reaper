...
7. **자가복구(옵션)**: lock 충돌 시 제한된 횟수로 재시도 가능 (기본 비활성)
8. **로깅**: `~/.mac-reaper/logs/YYYY-MM-DD.log` + macOS syslog
...
| `REAPER_RETRY_ON_LOCK_SKIP` | `0` | 1이면 lock 충돌 시 재시도 활성화 |
| `REAPER_RETRY_LOCK_MAX_ATTEMPTS` | `1` | lock 재시도 최대 횟수 |
| `REAPER_RETRY_LOCK_BACKOFF_SEC` | `5` | lock 재시도 간격(초) |
